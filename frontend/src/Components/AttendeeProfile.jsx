...
import {
  Box, Typography, Paper, Avatar, Stack, Divider,
...
function PasswordDialog({ open, onClose, userEmail }) {
  ...
  return (
    <Dialog
      ...
      PaperProps={{
        sx: {
          // ── KEY FIX: force dark bg, remove MUI's default white ──
          backgroundColor: `${T.paper} !important`,
    ...
    >
      ...
      >
        <Stack direction="row" spacing={1.5} alignItems="center">
          ...
          <Typography sx={{ color: T.textMain, fontWeight: 700, fontSize: "16px" }}>
            Update password
          </Typography>
        </Stack>
        <IconButton
          onClick={handleClose}
          sx={{
            color: T.textSub, borderRadius: "8px",
            border: `1px solid ${T.border}`, p: "5px",
            "&:hover": { color: T.textMain, borderColor: T.borderHover, bgcolor: "rgba(255,255,255,0.04)" },
          }}
        >
          <X size={15} />
        </IconButton>
        ...
        {done ? (
          ...
          <Stack alignItems="center" spacing={2} sx={{ py: 3, textAlign: "center" }}>
            <Box sx={{
              ...
              bgcolor: T.greenDim, border: `1px solid ${T.greenBorder}`,
              display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              <Check size={28} color={T.green} strokeWidth={2.5} />
            </Box>
            <Typography sx={{ color: T.textMain, fontWeight: 700, fontSize: "17px" }}>
              Password updated!
            </Typography>
            <Typography sx={{ color: T.textSub, fontSize: "13.5px", lineHeight: 1.7 }}>
              Your account is now secured with the new password.
            </Typography>
            <Button
              onClick={handleClose}
              sx={{
                mt: 1, bgcolor: T.greenDim, color: T.green,
          ...
          /* FORM STATE */
          <>
            <Typography sx={{ color: T.textSub, mb: 2.5, fontSize: "13.5px", lineHeight: 1.7 }}>
              Choose a strong, unique password to keep your account secure.
            </Typography>

            {/* New Password */}
            <Typography sx={{ fontSize: "10.5px", fontWeight: 700, color: T.textMuted, letterSpacing: "1px", textTransform: "uppercase", mb: "7px" }}>
              New password
            </Typography>
            <Box sx={{ position: "relative", mb: "10px" }}>
              <input
                ...
                style={{
                  ...
                  background: T.paperAlt, color: T.textMain,
              ...
              <IconButton
                ...
                sx={{ position: "absolute", right: "8px", top: "50%", transform: "translateY(-50%)", color: T.textMuted, p: "4px", "&:hover": { color: T.textSub } }}
              >
            ...
            <Button
              ...
              sx={{
                bgcolor: canSave ? T.accent : "rgba(255,255,255,0.05)",
                color: canSave ? "#07090F" : T.textMuted,
                border: `1px solid ${canSave ? T.accent : "rgba(255,255,255,0.07)"}`,
                fontWeight: 700, fontSize: "14.5px", py: "12px",
                borderRadius: "12px", textTransform: "none",
                transition: "all .2s",
                "&:hover": canSave ? { bgcolor: "#00bfca" } : {},
                "&.Mui-disabled": { opacity: 0.5, color: T.textMuted },
              }}
            >
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={1.5} alignItems="center">
        ...
        <Typography sx={{
          color: isMono ? T.textSub : T.textMain,
          fontWeight: 500,
          fontSize: isMono ? "12px" : "13.5px",
          wordBreak: "break-all",
...
export default function AttendeeProfile() {
  const [user, setUser]   = useState({ id: "", name: "", email: "", role: "attendee" });
  const [stats, setStats] = useState({ expos: 0, meetings: 0 });
  ...
  useEffect(() => {
    ...
    if (saved) {
      setUser({
        ...
        name:  saved.name  || "Attendee User",
        email: saved.email || "",
        role:  saved.role  || "attendee",
      });
      fetchStats(saved._id || saved.id);
    }
  }, []);
      ...
      <Container maxWidth="lg">
        ...
        <Paper elevation={0} sx={{ bgcolor: T.paper, border: `1px solid ${T.border}`, borderRadius: "22px", overflow: "hidden", mb: 3 }}>
          ...
          <Box sx={{ height: 175, bgcolor: T.paper, position: "relative", overflow: "hidden" }}>
            ...
            <Chip
              label="PLATFORM ATTENDEE"
              icon={<Zap size={13} color={T.accent} />}
              sx={{
                position: "absolute", top: 18, right: 20,
                bgcolor: "rgba(0,0,0,0.55)", color: T.accent,
                fontWeight: 800, fontSize: "10px", letterSpacing: "1.4px",
                border: `1px solid ${T.accentBorder}`,
                "& .MuiChip-icon": { ml: "8px" },
              }}
            />
          ...
          <Box sx={{ px: { xs: 3, md: "28px" }, pb: "28px" }}>
            <Box sx={{
              ...
              {/* ── AVATAR FIX: explicit bgcolor + color, no MUI override ── */}
              <Box sx={{
                width: 104, height: 104, borderRadius: "50%",
                border: `5px solid ${T.paper}`,
                bgcolor: "rgba(0,210,220,0.12)",
                boxShadow: `0 0 0 1px ${T.accentBorder}, 0 16px 40px rgba(0,0,0,0.6)`,
                display: "flex", alignItems: "center", justifyContent: "center",
                flexShrink: 0,
              }}>
                <Typography sx={{
              ...
              <Button
                ...
                sx={{
                  mb: { xs: 0, sm: "6px" },
                  bgcolor: "transparent", color: T.textMain,
                  border: `1px solid ${T.borderHover}`,
                  borderRadius: "10px", px: "20px", py: "9px",
                  textTransform: "none", fontWeight: 600, fontSize: "13px",
                  "&:hover": { borderColor: T.accent, color: T.accent, bgcolor: T.accentDim },
                  transition: "all .2s",
                }}
              >
                Security update
        ...
        <Grid container spacing={3}>
          ...
          {/* LEFT */}
          <Grid item xs={12} md={4} sx={{ p: 0, width: "220px" }}>
            <Stack spacing={3}>
              <Paper elevation={0} sx={{ p: "20px", bgcolor: T.paper, border: `1px solid ${T.border}`, borderRadius: "18px" }}>
                <Typography sx={{ color: T.textMuted, fontSize: "10.5px", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", mb: "16px" }}>
                  Activity metrics
                </Typography>
                <StatRow icon={<Layout size={15} />}      label="Events registered"  value={stats.expos}    isText={false} />
                <StatRow icon={<Calendar size={15} />}    label="Meetings confirmed" value={stats.meetings} isText={false} />
                <StatRow icon={<Fingerprint size={15} />} label="Security level"     value="Level 2"        isText={true}  />
              </Paper>
              ...
              <Paper elevation={0} sx={{ p: "14px 18px", bgcolor: T.greenDim, border: `1px solid ${T.greenBorder}`, borderRadius: "18px" }}>
                <Stack direction="row" spacing={2} alignItems="center">
                  ...
                  <Box>
                    ...
                    <Typography sx={{ color: "rgba(34,197,94,0.65)", fontSize: "12px", mt: "2px" }}>Optimized for networking</Typography>
                  </Box>
                </Stack>
              </Paper>
            </Stack>
          ...
          {/* RIGHT */}
          <Grid item xs={12} md={8} sx={{ p: 0, width: "650px" }}>
            <Paper elevation={0} sx={{ p: { xs: "20px", md: "26px" }, bgcolor: T.paper, border: `1px solid ${T.border}`, borderRadius: "18px", height: "100%" }}>
              ...
              <Grid container spacing={{ xs: 2.5, md: 3.5 }}>
                ...
                <Grid item xs={12} sm={6}>
                  <InfoField icon={<Briefcase size={16} />}   label="Designation" value="Business Attendee" />
                </Grid>
              </Grid>

              <Divider sx={{ my: 3, borderColor: T.border }} />

              <Box sx={{ p: "13px 15px", bgcolor: T.accentDim, borderRadius: "12px", border: `1px solid ${T.accentBorder}`, display: "flex", gap: "10px", alignItems: "flex-start" }}>
                ...
                <Box>
                  ...
                  <Typography sx={{ color: T.textSub, fontSize: "12.5px", lineHeight: 1.6 }}>
                    Bio editing and avatar upload will be available in the next system update.
                  </Typography>
                </Box>
              </Box>
            </Paper>
